...
from src.process.fmp import (FMPPriceProcessor,
                                    FMPNewsProcessor,
                                    FMPFeatureProcessor)
from src.process.alpaca import (AlpacaNewsProcessor,
                                       AlpacaPriceProcessor,
                                       AlpacaFeatureProcessor)
from src.process.akshare import (
    AkSharePriceProcessor,
    AkShareNewsProcessor,
    AkShareFeatureProcessor
)
from src.process.tushare import (
    TuSharePriceProcessor,
    TuShareNewsProcessor,
    TuShareFeatureProcessor
)
from src.process.binance import (
    BinancePriceProcessor,
    BinanceFeatureProcessor
)
from src.utils import assemble_project_path, get_tag_name, push_to_hub_folder
...
class Processor(AbstractProcessor):
    def __init__(self,
        ...
        self.processor_map = {
            ...
            "binance": {
                "price": BinancePriceProcessor,
                "feature": BinanceFeatureProcessor,
            }
        }
    ...
    async def run(self):
        ...
        tag_name = get_tag_name(assets_name=self.assets_name,
                                ...
                                data_type=self.data_type,
                                level=self.level)
        save_dir = os.path.join(self.workdir, tag_name)
        os.makedirs(save_dir, exist_ok=True)
...
class AggProcessor(AbstractProcessor):
    ...
    async def run(self):
        ...
        # Save the info
        info_path = os.path.join(self.workdir, "meta_info.json")
