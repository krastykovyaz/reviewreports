...
class TuSharePriceDownloader(AbstractDownloader):
    def __init__(self,
        ...
        self.pro = ts.pro_api(token=os.getenv("TUSHARE_API_KEY", None))
    ...
    async def run(self,
        ...
        if self.level == "1day":
            ...
            if "SS" in symbol:
                symbol_ = symbol.replace("SS", "SH")
            elif "SZ" in symbol:
                symbol_ = symbol.replace("SZ", "SZ")
            else:
                symbol_ = symbol

            df = self.pro.daily(ts_code=symbol_,
...
class TuShareNewsDownloader(AbstractDownloader):
    def __init__(self,
                 start_date: Optional[str] = None,
                 end_date: Optional[str] = None,
                 level: Optional[str] = None,
    ...
    async def run(self,
        ...
        raise NotImplementedError("TuShareNewsDownloader is not implemented yet.")