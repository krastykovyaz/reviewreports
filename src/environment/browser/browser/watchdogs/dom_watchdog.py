...
from src.environment.browser.dom.views import (
	EnhancedDOMTreeNode,
	SerializedDOMState,
)
from src.environment.browser.observability import observe_debug
from src.environment.browser.utils import time_execution_async

if TYPE_CHECKING:
	from src.environment.browser.browser.views import BrowserStateSummary, PageInfo
...
class DOMWatchdog(BaseWatchdog):
		...
		try:
			...
			if screenshot_b64 and content and content.selector_map and self.browser_session.browser_profile.highlight_elements:
				try:
					...
					from src.environment.browser.browser.python_highlights import create_highlighted_screenshot_async
	...
	async def _get_page_info(self) -> 'PageInfo':
		...
		from src.environment.browser.browser.views import PageInfo
