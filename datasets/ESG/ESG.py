...
class ESG_dataset(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        topic, level_name = self.config.name.split(separator)
        if level_name == "all":
            levels = LEVELS[topic]
        else:
        ...
        for split in ["test", "validation"]:
            root_file = dl_manager.download(os.path.join(split, "metadata.jsonl"))
            test_attached_files = {"": ""}
            with open(root_file, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if cur_line["Level"] in levels and cur_line["file_name"] != "":
                        ...
                        attached_file = dl_manager.download(os.path.join(split, attached_file_name))
